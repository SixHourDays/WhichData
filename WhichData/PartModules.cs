...
namespace WhichData
{
    ...
    /*
        ...
        //We deliberately hide some events, and replace them with ours
        public class WhichDataContainerHelper : PartModule
        {
            public ModuleScienceContainer m_container = null;
            public BaseEvent m_stockCollect;
            public BaseEvent m_stockReview;
            public BaseEvent m_stockStore;
            ...
            public void Start()
            {
                ...
                List<WhichDataContainerHelper> helpers = part.FindModulesImplementing<WhichDataContainerHelper>();
                List<ModuleScienceContainer> containers = part.FindModulesImplementing<ModuleScienceContainer>();
                int pairIndex = helpers.FindIndex(h => h == this);
                m_container = containers[pairIndex];
            ...
            public void CollectWhichData()
            {
                WhichData.instance.OnCollectWhichData(m_container);
            }
            ...
            public void StoreWhichData()
            {
                WhichData.instance.OnStoreWhichData(m_container);
            }
        ...
        //A part module that always accompanies a ModuleScienceExperiment
        //We deliberately hide some events, and replace them with ours
        public class WhichDataExperimentHelper : PartModule
        {
            public ModuleScienceExperiment m_experi = null;
            ...
            public void Start()
            {
                if ( m_experi != null) { return; }

                List<WhichDataExperimentHelper> helpers = part.FindModulesImplementing<WhichDataExperimentHelper>();
                List<ModuleScienceExperiment> experiments = part.FindModulesImplementing<ModuleScienceExperiment>();
                int pairIndex = helpers.FindIndex(h => h == this);
                m_experi = experiments[pairIndex];

                pairIndex += 1;
                Debug.Log("GA Experiment Helper " + pairIndex + "/" + helpers.Count + " for " + m_experi.name);

                m_stockCollect = m_experi.Events["CollectDataExternalEvent"];
                m_stockReview = m_experi.Events["ReviewDataEvent"];
                ...
                {
                    //the kerb to kerb external soil & eva reports dont work anyway, so hide them
                    m_experi.Events["DeployExperimentExternal"].guiActiveUnfocused = false;
                    //I think squad forgot to disable kerb's experi's external collect events.  The uninitialized name "Take Data" furthers the theory.
                    //Pods display only a single summed Take Data n on external collect.
                    //EVA kerb's also display that, but accidentally show a Take Data per experiment too.
                    //So we disable the per-experiment Take Data.
            ...
            {
                //dont pass along to stock event, we've no use for the ERD to popup just to be killed again
                WhichData.instance.OnReviewData(m_experi);
            }
